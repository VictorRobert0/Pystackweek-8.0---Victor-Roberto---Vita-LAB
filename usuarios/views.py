...
def cadastro(request):
    if request.method == "GET": 
        ...
        if not senha == confirmar_senha:
            messages.add_message(request, constants.ERROR, 'Por favor, digite as senhas iguais')
            return redirect('/usuarios/cadastro')
        
        if len(senha) < 6:
            messages.add_message(request, constants.ERROR, 'Por favor, digite uma senha MAIOR que 6 digitos')
            return redirect('/usuarios/cadastro')
